...
describe Rectangle do
  context "Rectangle to have a  perimeter" do
    it "perimeter should be 30 for length 5 and breadth 3 in cm" do
      rect=Rectangle.new(Length.new(5, Unit.new(Unit::CM)), Length.new(3, Unit.new(Unit::CM)))
      expected_perimeter = Length.new(160, Unit.new(Unit::MM))
    ...
    it "perimeter should be 10600 for length 5m and breadth 3cm" do
      rect=Rectangle.new(Length.new(5, Unit.new(Unit::M)), Length.new(3, Unit.new(Unit::CM)))
      expected_perimeter = Length.new(10060, Unit.new(Unit::MM))
      expect(rect.perimeter.length).to eq(expected_perimeter.length) and expect(rect.perimeter.length).to eq(expected_perimeter.length)
    end

    it "perimeter should be 900 for length 3cm and breadth 150mm" do
      rect=Rectangle.new(Length.new(3, Unit.new(Unit::CM)), Length.new(150, Unit.new(Unit::MM)))
      expected_perimeter = Length.new(360, Unit.new(Unit::MM))
      expect(rect.perimeter.length).to eq(expected_perimeter.length) and expect(rect.perimeter.length).to eq(expected_perimeter.length)
    end
  end
  ...
  context "equality check for rectangles" do
    it "Rectangle with length 3 and breadth 4 should be same as Rectangle with length 3 and Breadth 4" do
      rect1=Rectangle.new(Length.new(3, Unit.new(Unit::MM)), Length.new(4, Unit.new(Unit::MM)))
      rect2=Rectangle.new(Length.new(3, Unit.new(Unit::MM)), Length.new(4, Unit.new(Unit::MM)))
  ...
  context "Equality and type check" do
    ...
    it 'equates to value of Length object' do
      length1 = Length.new(1, Unit.new(Unit::M))
      length2 = Length.new(1, Unit.new(Unit::M))
      expect(length1).to eq(length2)
    end

    it 'same Length should be equal' do
      length1 = Length.new(1, Unit.new(Unit::CM))
      expect(length1).to eq(length1)
    end

    it '2 Length added should have same type' do
      length1 = Length.new(1, Unit.new(Unit::MM))
      length2 = Object.new
    ...
    it 'Length should not be zero/nil' do
      length1 = Length.new(0, Unit.new(Unit::M))
      expect(length1).to_not eq(nil)
    end

    it "symmetric property" do
      length1 = Length.new(1, Unit.new(Unit::MM))
      length2 = Length.new(1, Unit.new(Unit::MM))
    ...
    it "transitive property" do
      length1 = Length.new(1, Unit.new(Unit::MM))
      length2 = Length.new(1, Unit.new(Unit::MM))
      length3 = Length.new(1, Unit.new(Unit::MM))
